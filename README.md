...
2. In the SQL editor, run `supabase/migrations/0001_init.sql` (or `supabase db push` if you have
   the Supabase CLI linked to the project).
3. Copy your Project URL and anon public key into `.env.local`.
4. (Optional) In Authentication settings, disable "Confirm email" for faster local testing, or
   leave it on and use the confirmation email flow.
...
- [ ] `npm install && npm run build` run successfully in your environment (please verify — not
      run here due to sandbox network restrictions)
- [ ] Live Supabase project created and migration applied
